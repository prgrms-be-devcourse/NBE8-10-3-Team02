...
class TagController(
    ...
    fun tags(): List<TagDto> {
        return tagService.findAll().map { TagDto(it) }
    }

    @PostMapping
    fun create(@RequestBody req: CreateTagRequest): RsData<TagDto> {
    ...
    fun createFromIgdb(@PathVariable igdbId: Long): RsData<List<TagDto>> {
