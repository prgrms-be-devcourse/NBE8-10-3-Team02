...
class GameSearchRepositoryImpl(
    ...
    private fun genreIn(genreIds: List<Long>?): BooleanExpression? {
        return if (genreIds.isNullOrEmpty()) null
        else gameGenre.genre.igdbId.`in`(genreIds)
    }

    private fun platformIn(platformIds: List<Long>?): BooleanExpression? {
        return if (platformIds.isNullOrEmpty()) null
        else gamePlatform.platform.igdbId.`in`(platformIds)
    }
}