...
class ApiV1GameController(
    ...
    fun getGameDetail(@PathVariable igdbId: Long): GameDetailResponse? {
    ...
    fun getVideoId(@PathVariable igdbId: Long): GameVideoResponse? {
    ...
    fun getSimilarGames(@PathVariable igdbId: Long): List<SimilarGameResponse?> {
    ...
    ): List<PopularGameCardDto?> {
        return gameService.getIgdbPopularGames(limit) ?: emptyList()
    }
    ...
    fun search(
        ...
        val condition = GameSearchCondition().apply {
    ...
    fun getGenres(): List<GenreResponse?> {
        return genreService.getGenres() ?: emptyList()
    }
